...
import { Mail, Phone, MapPin, Calendar, Save, Camera, KeyRound, Lock, Eye, EyeOff, Shield, Edit2 } from 'lucide-react';
...
function AdminControlsSection() {
  const { adminPassword, updateAdminPassword, isPlanActive, triggerPlanBlink } = useApp();
  const [open, setOpen] = useState(false);
  const [form, setForm] = useState({ current: '', next: '', confirm: '' });
  const [show, setShow] = useState({ current: false, next: false, confirm: false });
  const [status, setStatus] = useState(null);
  ...
  async function handleUpdate() {
    ...
    // Check if current password matches management password OR account password
    let isValid = false;
    
    // First check if it matches management password
    ...
    if (!isValid) {
      setStatus('error');
      setMsg('Current password is incorrect. Enter either your management password or account password.');
      return;
    }
    ...
    <div>
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 1 }}>Enter the management password to unlock management controls</div>
        </div>
      </div>
      ...
      {open && (
        <div style={{ marginTop: 12, padding: '18px 20px', background: 'var(--bg-subtle)', borderRadius: 14, border: '1.5px solid #C7D4FF', display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div>
            <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.5px', display: 'block', marginBottom: 6 }}>Current Password (Management or Account)</label>
            <div style={{ position: 'relative' }}>
              <div style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)' }}>
                <KeyRound size={14} color="#9CA3AF" />
              </div>
            ...
            <div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 4, fontStyle: 'italic' }}>Enter your current management password or account password</div>
          </div>
          ...
          <PwdInput label="Confirm New Password"    fieldKey="confirm" form={form} setForm={setForm} show={show.confirm} toggle={() => setShow(p => ({ ...p, confirm: !p.confirm }))} accentColor="#3B5BFC" />
          <StatusMsg status={status} msg={msg} />
          <button onClick={handleUpdate} style={{ alignSelf: 'flex-start', display: 'flex', alignItems: 'center', gap: 8, padding: '9px 20px', background: 'linear-gradient(135deg, #3B5BFC, #2142D9)', border: 'none', borderRadius: 10, color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', boxShadow: '0 4px 12px rgba(59,91,252,0.3)' }}>
            <Shield size={13} /> Update Management Password
          </button>
        </div>
      )}
    </div>
...
export default function ManagementProfile({ member }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      // Don't save base64 data URLs to Firestore - only save Firebase Storage URLs
      const updateData = {
        name: form.name,
        phone: form.phone,
      ...
      // Only include avatarImg if it's a Firebase Storage URL (not base64)
  ...
  function handleAvatarChange(e) {
    ...
    // Show preview immediately (but don't save to Firestore yet)
    const reader = new FileReader();
    reader.onload = ev => {
      setAvatarImg(ev.target.result); // Local preview only
    };
      ...
      import('../../lib/storageService').then(({ uploadUserAvatar }) => {
        uploadUserAvatar(file, uid)
          .then(async (downloadURL) => {
            setAvatarImg(downloadURL); // Update with Firebase Storage URL
            
            // Update profile with new avatar URL (not base64)
            const { updateProfile } = await import('../../lib/userProfileService');
    ...
    <div style={{ flex: 1, overflow: 'auto', padding: '24px 28px', display: 'flex', flexDirection: 'column', gap: 18 }}>
      <div style={{ background: 'var(--bg-surface)', borderRadius: 18, padding: '28px', border: '1.5px solid var(--border)', transition: 'background 0.25s, border-color 0.25s' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginBottom: 24 }}>
          <div onClick={handleAvatarClick} title={editing ? 'Click to change photo' : undefined}
          ...
          <button onClick={handleEditToggle} style={{ padding: '7px 16px', borderRadius: 10, border: `1.5px solid ${editing ? '#7C3AED' : 'var(--border)'}`, background: editing ? '#F5F3FF' : 'var(--bg-surface)', fontSize: 12, fontWeight: 600, color: editing ? '#7C3AED' : 'var(--text-secondary)', cursor: 'pointer', transition: 'all 0.15s', display: 'flex', alignItems: 'center', gap: 6 }}>
            <Edit2 size={14} />
            {editing ? 'Cancel' : ''}
          </button>
        </div>
