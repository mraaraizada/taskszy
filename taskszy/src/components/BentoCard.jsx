    ...
    <div className="flex-1 rounded-xl border border-border/40 flex flex-col bg-background/50 overflow-hidden">
      ...
      <div className="flex-1 p-1 overflow-y-auto scrollbar-hide">
        {[
          ...
          >
            <div className={cn("w-6 h-6 rounded-md border border-border/40 flex items-center justify-center transition-colors", item.bgColor)}>
              <HugeiconsIcon icon={item.icon} size={12} className={item.iconColor} />
            </div>
            <div className="flex flex-col min-w-0 flex-1">
              <span className="text-[10px] font-medium text-foreground truncate">
                {item.file}
...
export const DashboardPreviewInternal = () => {
  const [activeTab, setActiveTab] = useState(TABS[0]);

  // Auto-rotate tabs every 2 seconds
  useEffect(() => {
  ...
  return (
    <div className="relative w-full h-[260px] sm:h-[300px] overflow-hidden rounded-2xl sm:rounded-[2rem]">
      <div className="absolute top-12 left-12 w-full h-full bg-muted rounded-3xl border border-border/50 opacity-80" />
      <div className="absolute top-8 left-20 w-full h-full bg-background rounded-tl-3xl shadow-xl flex flex-col overflow-hidden ring-1 ring-gray-300">
        <div className="px-5 py-4 rounded-tl-3xl border-b border-border/70 flex items-center relative backdrop-blur-sm">
          ...
          <div className="absolute left-1/2 -translate-x-1/2 flex items-center gap-2">
            <span className="text-xs text-foreground uppercase font-semibold">
              Workspace
            </span>
          </div>
        </div>
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="flex-1 bg-background p-5 pt-6 flex flex-col gap-4 overflow-hidden relative">
            <header className="flex flex-col gap-0.5">
              <h3 className="text-xs font-semibold text-primary tracking-tight line-clamp-1 uppercase">
                {activeTab.header}
              </h3>
