...
console.log('💬 FeedbackPage.jsx loaded at:', new Date().toISOString());
...
export default function FeedbackPage() {
  ...
  useEffect(() => {
    console.log('💬 FeedbackPage: Initializing, loading feedback data...');
    loadFeedback(true); // Reset on initial load to avoid duplicates
  }, []);

  const loadFeedback = async (reset = false) => {
    try {
      console.log('💬 FeedbackPage: Fetching feedback from optimized service...');
      setLoading(true);
      ...
      console.log('✅ FeedbackPage: Feedback loaded successfully', {
      ...
      setLoading(false);
      console.log('💬 FeedbackPage: Loading complete, state updated');
    }
  };

  const handleSendFeedbackRequest = async (organizationId, organizationName) => {
    try {
      console.log('📤 FeedbackPage: Sending feedback request', {
        organizationId,
        organizationName
      });
      await sendFeedbackRequest(organizationId, 'We would love to hear your feedback!');
      console.log('✅ FeedbackPage: Feedback request sent successfully');
      toast.success('Feedback');
      setShowFeedbackDialog(false);
      setSelectedOrganization(null);
      
      // Reload feedback data after sending request
      console.log('🔄 FeedbackPage: Reloading feedback after request sent...');
      loadFeedback(true);
    } catch (error) {
      console.error('❌ FeedbackPage: Error sending feedback request:', error);
  ...
  console.log('📄 FeedbackPage: Pagination calculated', {
    totalFeedback: filteredFeedback.length,
    rowsPerPage,
    totalPages,
    currentPage,
    displayedFeedback: paginatedFeedback.length,
    containerHeight,
    loading
  });
