...
export async function getAllOrganizations(forceRefresh = false) {
  ...
  if (!forceRefresh) {
    ...
    if (cached) {
      console.log('✅ organizationService: Returning cached organizations (0 reads)');
      return cached; // Return same reference to prevent unnecessary re-renders
    }
  }
  
  try {
    console.log('🔄 Fetching organizations from Firebase...');
      ...
      snapshot.docs.map(async (workspaceDoc) => {
        ...
        if (workspaceData.ownerId) {
          try {
            ...
            console.warn(`❌ Could not fetch owner data for workspace ${workspaceId}:`, err);
          }
        }
        ...
        try {
          ...
          console.warn(`Could not fetch team count for workspace ${workspaceId}:`, err);
        }
        ...
        try {
          ...
          console.warn(`Could not fetch tasks count for workspace ${workspaceId}:`, err.message);
        }
        ...
        if (workspaceData.customPlanRequest) {
          console.log(`📋 Found customPlanRequest for workspace ${workspaceId}:`, workspaceData.customPlanRequest);
        }
    ...
    console.log(`✅ Fetched ${organizations.length} organizations from Firebase`);
    
    // Cache the results
    cacheManager.set(CACHE_KEYS.ORGANIZATIONS, organizations);
    console.log('✅ Organizations cached successfully (TTL: 6 hours)');
