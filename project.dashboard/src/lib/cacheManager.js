...
class CacheManager {
        ...
        if (key.startsWith(this.storagePrefix)) {
          ...
          if (stored) {
            ...
            if (parsed.timestamp && (Date.now() - parsed.timestamp < parsed.ttl)) {
              ...
              console.log(`💾 Restored cache from localStorage: ${cacheName}`);
            }
          }
        }
      });
  ...
  saveToStorage(name, cache) {
    try {
      ...
      console.warn(`Failed to save cache ${name} to localStorage:`, err);
    }
  }
  ...
  createSetMethod(name) {
    return function(data) {
      ...
      if (hasChanged) {
        console.log(`📦 Cache SET: ${name} (Data changed)`);
      } else {
        console.log(`📦 Cache SET: ${name} (No changes detected)`);
      }
  ...
  createClearMethod(name) {
    return function() {
      ...
      try {
        ...
        console.warn(`Failed to remove cache ${name} from localStorage:`, err);
      }
