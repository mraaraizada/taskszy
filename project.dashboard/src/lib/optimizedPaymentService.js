...
export async function getCustomRequestsPaginated(options = {}) {
  ...
  try {
    console.log('🔍 Fetching custom requests from Firestore...');
    ...
    try {
      ...
      console.log('📊 Trying query with orderBy createdAt...');
      snapshot = await getDocs(q);
      console.log('✅ Query successful, got', snapshot.size, 'documents');
    } catch (orderError) {
      console.warn('⚠️ OrderBy failed, using simple query:', orderError.message);
      ...
      snapshot = await getDocs(q);
      console.log('✅ Simple query successful, got', snapshot.size, 'documents');
    }
    ...
    if (status) {
      ...
      console.log('📊 Filtered to', requests.length, 'requests with status:', status);
    }
    
    console.log('✅ Returning', requests.length, 'custom requests');
