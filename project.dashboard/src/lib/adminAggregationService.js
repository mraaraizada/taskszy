...
export async function subscribeToAdminAggregation(callback) {
  try {
    ...
    const unsubscribe = onSnapshot(aggRef, (snap) => {
      if (snap.exists()) {
        console.log('📊 Admin stats loaded from aggregation (1 read)');
        callback(snap.data());
      } else {
        callback(null);
      }
      ...
      if (error.code === 'permission-denied') {
        console.log('ℹ️ Admin aggregation not available, using cached data');
      } else {
        console.warn('⚠️ Admin aggregation listener error:', error.message);
      }
      callback(null);
    });
...
export async function getAdminAggregation() {
  try {
    ...
    if (aggSnap.exists()) {
      console.log('📊 Admin aggregation loaded (1 read)');
      return aggSnap.data();
    }
    ...
    if (error.code === 'permission-denied') {
      ...
      console.warn('⚠️ Could not load admin aggregation:', error.message);
    }
    return null;
  }
}
...
export async function rebuildAdminAggregation() {
  console.log('🔄 Rebuilding admin aggregation...');
  ...
  try {
    ...
    console.log(`📊 Loaded ${workspacesSnap.size} workspaces`);
    ...
    for (const workspaceDoc of workspacesSnap.docs) {
      ...
      try {
        ...
        console.warn(`Could not fetch tasks count for ${workspaceId}`);
      }
    ...
    console.log('✅ Admin aggregation rebuilt successfully:', {
    ...
    console.error('❌ Error rebuilding admin aggregation:', error);
    throw error;
  }
}
...
export async function getAdminAggregationWithFallback() {
  ...
  if (aggregation) {
    console.log('✅ Using admin aggregation (1 read)');
    return aggregation;
  }
  
  // Fallback: Use optimized organization service to calculate stats from cache
  console.log('⚠️ Aggregation not found, calculating from cached data...');
  try {
    ...
    console.error('❌ Error calculating stats from cache:', error);
    // Return empty stats
    return {
